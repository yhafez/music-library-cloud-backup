	...
	async (req: Request, res: Response, next: NextFunction) => {
		...
		try {
			if (await getFileByName(fileName)) {
				return next(new AppError('File already exists in S3 bucket.', 409))
			}

			let metadata: Metadata
			try {
				const metadataResults = await parseBuffer(fileContent, { mimeType: req.file.mimetype })
				metadata = {
					title: metadataResults.common.title || '',
					artist: metadataResults.common.artist || '',
					album: metadataResults.common.album || '',
					genre: metadataResults.common.genre || [],
					key: metadataResults.common.key || '',
					bpm: metadataResults.common.bpm || 0,
				}
			} catch (err) {
			...
			if (result.$metadata.httpStatusCode !== 200) {
				await query('ROLLBACK')
				return next(new AppError(`Failed to upload file to S3`, result.$metadata.httpStatusCode))
			}
...
s3Router.delete('/delete/:id', async (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		if (!fileName) {
			return next(new AppError('File already exists.', 409))
		}
